<template>
  <section class="footer-navigation">
    <router-link :to="{ name: 'home' }"><i class="fas fa-home nav-btn"></i></router-link>
    <router-link :to="{ name: 'cart' }"><i class="fas fa-shopping-cart nav-btn"></i></router-link>
    ...
    <div class="notification-wrapper">
      ...
      <transition name="slide-fade">
        ...
        >
          <i :class="notificationIcon" :style="{ color: notificationColor }"></i>
          ...
          <div v-if="notifications.length > 0">
            ...
            >
              <span :style="{ color: getColor(notification.type) }">
                <i :class="getIcon(notification.type)" :style="{ color: getColor(notification.type) }"></i>
                {{ notification.message }}
              </span>
              <button class="delete-btn" @click="removeNotification(notification.id, index, notification)">✖</button>
            </div>
    ...
    <router-link :to="{ name: 'profile' }"><i class="fas fa-user nav-btn"></i></router-link>
  </section>
</template>
...
const noNotificationsMessage = ref(false); 

/*************** Notification Style ***************/
const notificationColor = ref('#333');
const notificationIcon = ref('fas fa-bell');

let popoverTimeout = null; // Timeout to handle multiple notifications
...
function setTempNotification(notification) {
  ...
  tempNotifications.value.push(notification);
  ...
  popoverTimeout = setTimeout(() => {
    ...
    latestNotification.value = null; // Clear latestNotification after use
  }, 3000);
}
...
function toggleNotificationBox() {
  ...
  if (notifications.value.length === 0) {
    noNotificationsMessage.value = true;
  } else {
    noNotificationsMessage.value = false;
  }
}
...
|----------------------------------------------- 
*/
async function removeNotification(id, index, notification) {
  // Call store function to remove notification from DB
  if (notification in tempNotifications.value) {
    notifications.value.splice(index, 1); // Remove from local array
  } else {
...
async function clearNotifications() {
  await notificationStore.clearAllNotifications(); // Clear all from DB
  notifications.value = [];
...
*/
onMounted(async () => {
  await notificationStore.fetchNotifications(); // Fetch notifications
  notifications.value = notificationStore.getNotifications;

  // Get the latest notification if available
  const latest = notificationStore.getLatestNotification;
  if (latest && latest.length > 0) {
    setTempNotification(latest[0]); // Using the first notification as the latest
  }
});
</script>
